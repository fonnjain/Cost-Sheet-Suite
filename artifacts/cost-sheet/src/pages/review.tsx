...
function RmPriceSource({ quote }: { quote: Quote }) {
                ...
                <Table>
                  <TableHeader className="bg-card/50">
                    <TableRow className="border-border/50">
                      ...
                      <TableHead>RM Price Source</TableHead>
                      <TableHead className="text-right">Quote Price / MT</TableHead>
                      {sortedQuotes.some((q) => q.discountValue != null) && (
                        <>
                          <TableHead className="text-right">Discount</TableHead>
                          <TableHead className="text-right">Net Price / MT</TableHead>
                        </>
                      )}
              ...
              <CardContent className="p-0">
                <Table>
                    ...
                    <TableBody>
                      ...
                      {sortedQuotes.some((q) => q.discountValue != null) && (
                        <>
                          <TableRow className="border-border/40 bg-primary/3 hover:bg-primary/5">
                            <TableCell className="sticky left-0 bg-card z-10 text-primary font-medium">− Discount</TableCell>
                            {sortedQuotes.map((q) => {
                              const discountAmt = q.discountValue != null && q.discountMode != null
                                ? q.discountMode === 'pct'
                                  ? q.quotePricePerMt * q.discountValue
                                  : q.discountValue
                                : null;
                              return (
                                <TableCell key={q.id} className="text-right font-mono text-sm text-primary whitespace-nowrap">
                                  {discountAmt != null
                          ...
                          <TableRow className="border-t border-emerald-500/30 bg-emerald-500/5 hover:bg-emerald-500/5">
                            <TableCell className="sticky left-0 bg-card z-10 font-bold text-emerald-400">Net Quote /MT</TableCell>
                            {sortedQuotes.map((q) => (
                              <TableCell key={q.id} className="text-right font-mono font-bold text-emerald-400 whitespace-nowrap">
                                {q.netQuotePricePerMt != null
                                  ? formatINR(q.netQuotePricePerMt)
                ...
                <Table>
                  <TableHeader className="bg-card/50">
                    <TableRow>
                      ...
                      <TableHead className="text-right">Quote Price / MT</TableHead>
                      {quotes?.some((q) => q.discountValue != null) && (
                        <>
                          <TableHead className="text-right">Discount</TableHead>
                          <TableHead className="text-right">Net Price / MT</TableHead>
                        </>
                  ...
                  <TableBody>
                    {quotes?.map((quote) => {
                      const hasAnyDiscount = quotes.some((q) => q.discountValue != null);
                      const discountAmt = quote.discountValue != null && quote.discountMode != null
                        ? quote.discountMode === 'pct'
                          ? quote.quotePricePerMt * quote.discountValue
                          : quote.discountValue
                        : null;
                      return (
                        ...
                        {hasAnyDiscount && (
                          <>
                            ...
                            <TableCell className="text-right font-mono font-bold text-emerald-400">
                              {quote.netQuotePricePerMt != null
                                ? formatINR(quote.netQuotePricePerMt)
                                : <span className="text-muted-foreground font-normal">—</span>
                              }
                            </TableCell>
                          </>
